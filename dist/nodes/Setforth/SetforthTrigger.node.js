...
class SetforthTrigger {
    ...
    webhookMethods = {
        default: {
            async checkExists() {
                ...
                try {
                    const webhooks = await GenericFunctions_1.setforthApiRequest.call(this, 'GET', '/webhooks');
            ...
            async create() {
                ...
                try {
                    const responseData = await GenericFunctions_1.setforthApiRequest.call(this, 'POST', '/webhooks', body);
                    const webhookData = this.getWorkflowStaticData('node');
            ...
            async delete() {
                ...
                if (webhookData.webhookId !== undefined) {
                    try {
                        await GenericFunctions_1.setforthApiRequest.call(this, 'DELETE', `/webhooks/${webhookData.webhookId}`);
                    }
                    catch (error) {
                        return false;
                    }
    ...
    async webhook() {
        ...
        const body = req.body;
        const signature = req.headers['x-setforth-signature'];
        if (signature) {
        }
        return {
